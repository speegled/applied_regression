...
::: example
We show how to do several common types of tasks with this (simulated) data set. Before doing inference, we should check the residuals.

```{r out.width=c('50%', '50%'), fig.show="hold", fig.cap="Residual plots for simulated data look fine."}
plot(mod2)
```

Since they are fine, we can proceed. If we want a 99 percent confidence interval for $\beta_2$, we would use `confint` as follows:

```{r}
confint(mod2, parm = "ws", level = 0.99)
```

A 99 percent confidence interval for $\beta_2$ is $[1.895, 3.895]$.

If we want to test $\beta_1 = 0$ versus $\beta_1\not= 0$, we look at the $p$-value associated with `vs`. We see that it is less than .05, so we reject that $\beta_1 = 0$ and conclude that it is not zero.

Suppose we want a 95 percent confidence interval for the mean value of the response when `vs = 5` and `ws = 5`. We would use

```{r}
predict(mod2, newdata = data.frame(vs = 5, ws = 5), interval = "conf")
```

We see that we are 95 percent confident that the **mean** response is between 21.57 and 25.78 with those values of `vs` and `ws`. However, if we were imagining that we were going to get a single additional observation and we want a range of values that we are 95 percent confident that the single additional value will fall in, we would do this:

```{r}
predict(mod2, newdata = data.frame(vs = 5, ws = 5), interval = "pre")
```

We get quite a different answer: the next obserevation will be between 13 and 34 with 95 percent confidence. Note that these prediction intervals take into account both the uncertainty in our estimates for the parameters, **and** the inherent uncertainty when sampling a single value.
...
One tool for assessing the fit of a model to a data set is the Akaike Information Criterion (AIC). The AIC is a number associated with a model of a data set, for which **lower** numbers correspond to **better** models. The formula is
\[
AIC = -2 \log \mathrm{likelihood} + 2k
\]
where $k$ is the number of parameters included in the model, and the likelihood is the likelihood of the data when using the MLE estimators for the parameters. As we can see from the formula, if we add one parameter to the model, the AIC will go down if the log likelihood increases by 1 or more when adding the parameter. Adding parameters will not decrease the likelihood, so this criterion is balancing how much the parameter increases the likelihood against the value 1.

To understand this, we need to understand lilekihood functions, which are widely used in statistics.

### Likelihood functions

Consider a random sample $x_1, \ldots, x_n$ from a normal distribution with unknown mean and standard deviation. The *likelihood* of the sample is given as a function of both $\mu$ and $\sigma$ by
\[
\prod \ttext{dnorm} (x_i, \mu, \sigma)
\]
If we choose $\mu$ and $\sigma$ to maximize the likelihood of the data, then we would take the partials with respect to $\mu$ and $\sigma$ and set equal to zero, then solve the equations. We obtain
\[
\hat \mu = \overeline{x}
\]
and
\[
\hat \sigma^2 = \frac 1n \sum_{i = 1}^n(x_i - \overline{x})^2
\]
Note that we do **not** obtain the unbiased estimator for $\sigma^2$, which has $n - 1$ in the denominator. We call these estimators the *maximum likelihood estimators*. 

One can similarly show that the maximum likelihood estimators for $\hat \beta_i$ and $\sigma$ in a linear regression model are as follows. The MLE for $\hat \beta_i$ are the **same** as what we obtained when we minimized the SSE. The MLE for $\hat \sigma^2$ is $\frac 1n \sum_{i = 1}^n (\hat \epsilon_i - \overline{\epsilon})^2$. Since $\overline{\epsilon} = 0$, this is usually written as $\frac 1n \sum_{i = 1}^n \epsilon_i^2$.
