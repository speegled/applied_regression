...
bind_rows(
  tidy(mod_naive, conf.int = TRUE) |> mutate(model = "Naive"),
  tidy(mod_match, conf.int = TRUE) |> mutate(model = "Matched")
) |>
  filter(term == "treatTreated") |>
  select(model, estimate, conf.low, conf.high, p.value)
```

The comparison between the naive and matched estimates shows how much the answer changes once we restrict attention to more comparable treated and control individuals. If the two estimates differ substantially, that is evidence that baseline differences between the groups mattered.
