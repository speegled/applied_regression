...
## This is all from last semester
...
Sometimes that is appropriate, but if we are thinking of the response as a 0/1 random variable, then its expected value is the **probability** of a success. The affine equation above can be negative or bigger than 1, which would be impossible for a probability. Alternatively, let $p_i = E[y_i]$, and we could model the **log odds** of the response as an affine equation of the predictors (we denote the number of predictors by $P$ now):

\[
\log \frac{p_i}{1-p_i} = \beta_0 + \sum_{j = 1}^P \beta_j x_{ij}
\]

This is the approach that is taken in logistic regression. For completeness, let's solve the above equation for $p_i$:

\[
p_i = \frac{e^{\beta_0 + \sum_{j = 1}^P \beta_j x_{ij}}}{1 + e^{\beta_0 + \sum_{j = 1}^P \beta_j x_{ij}}} = \frac{1}{1 + e^{-\beta_0 - \sum_{j = 1}^P \beta_j x_{ij}}}
\]

The *error* component in this model is not as easy to see as in the normal case, because it isn't simply added as a random variable. But we can see that the random component of the response is binomial with $n = 1$ and $p_i = \frac{1}{1 + e^{-\beta_0 - \sum_{j = 1}^P \beta_j x_{ij}}}$. This is is often referred to as "binomial error" or "binomial noise", but the term *random component* makes more sense to me. The predictors can be either continuous or categorical, but we will be going through the derivations only for a single continuous predictor. One final piece of terminology is the *link function*. In the case of logistic regression, the link function is $\log \frac{x}{1-x}$. This is the link between the linear combination of predictors and the expected value of the response. The inverse link function is the inverse function of $\log \frac{x}{1-x}$, which is $\frac{1}{1 + e^{-x}}$. This is the function that links the linear combination of predictors to the expected value of the response. The R functions which implement those functions are `qlogis` and `plogis`, respectively.
...
\[
\frac{1}n \sum_{i = 1}^n \Phi(p_i y_i)
\]
...
```{r, echo=FALSE}
dd <- read.csv("/Users/speegled/Documents/teachingold/Spring2020/4870/data/Example5.1Data.csv")
summary(dd)
```
...
(1 - \pi) e^{-\lambda} \lambda^y/y!& y = 1, 2, \ldots
\end{cases}
\]
...
```{r}
mod_pois <- glm(art ~ ., data = bioChemists, family = poisson)
summary(mod_pois)
```

We see that this is unlikely to be a good model, because of overdispersion. There may also be other reasons this is not a good model!

Let's try negative binomial.

```{r}
mod_nb <- MASS::glm.nb(art ~ ., data = bioChemists)
summary(mod_nb)
plot(mod_nb, which = 5)
```
...
```{r}
mod_zif_nb <- zeroinfl(art ~ . | ., data = bioChemists, dist = "negbin")
summary(mod_zif_nb)
```

The residuals look worse in this model, but notice that they are the **Pearson** residuals and not the **deviance residuals**. To compare apples to apples, let's look at the Pearson residuals in the original negative binomial model. These seem more or less the same as the model we built.

```{r}
summary(residuals(mod_nb, type = "pearson"))
```

There are several things that we need to do now. We should remove predictors that aren't significant to the model, and we also need to figure out which model we should be using. I don't have definitive answers for you on this.

1. I have seen it argued that negative binomial models are already flexible enough that we don't normally need to have zero inflated negative binomial responses. 
2. Some people use the Vuong test to determine whether to use the zero inflated model. Others have problems with the Vuong test. The current consensus seems to be that it is inappropriate to use it, but you shouldn't be surprised to see it [in the wild](https://stats.idre.ucla.edu/r/dae/zip/). I will not discuss how to use it or interpret it.
3. That leaves us with few options to evaluate our models! Let's look at the number of zeros that we have in our data, compared to the number of zeros that our models predict. I would point out that this has low power, but it does test directly what we are trying to do.
4. One last option is to base the model on expert opinion, and see whether any of the predictors are significant for the zero inflation part. This is also tricky.
...
```{r}
summary(mod_nb)
head(predict(mod_nb, type = "response")) #means
rnbinom(915, size = 2.264387695, mu = predict(mod_nb, type = "response"))
```

```{r}
zeros <- replicate(1000, {
  sum(rnbinom(915, 
          size = 2.264387695, 
          mu = predict(mod_nb, type = "response")) == 0)
})
quantile(zeros, c(.025, .975))
sum(bioChemists$art == 0)
```

This indicates that the negative binomial model without zero inflation seems to be doing an OK job modeling the number of zeros in the data set. Just as a test, let's add a bunch of structural zeros and see whether it then fails this test.
...
```{r}
mod_zifbio_nb <- zeroinfl(art ~ . | ., data = zif_bio, dist = "negbin")
summary(mod_zifbio_nb)
```


```{r}
vuong(mod_nb, mod_zif_nb)
```

We see that the Vuong test is non-commital about whether the zero inflated model is better. It depends on which version of the test that you use.
