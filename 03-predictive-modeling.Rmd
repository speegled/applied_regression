...
In this section, though, we will thouroughly examine the estimates for MSE in the case of a simple linear model $y = 1 + 2x + \epsilon$, where $\epsilon \sim N(0, \sigma = 3)$. We suppose that we have data $\{(x_1, y_1), \ldots, (x_N, y_N)\}$, and that future $x$ values are generated through a random process with pdf $f$. It can be shown that the expected MSE of the model is given by
\[
\sigma^2 + \sigma^2/N + \sigma^2 \int \frac{(x - \overline{x})^2}{S_{xx}} f(x)\, dx
\]
where $S_{xx} = \sum_{i = 1}^N (x_i - \overline{x})^2$. In the special case that the original $x_i$ are uniformly sampled from 0 to $M$ (ie `seq(0, M, length.out = N)` and new $x$ values are chosen uniformly over the interval $[0, M]$ we get an expected MSE of 
\[
\sigma^2\bigl(1 + 1/N + \frac{M^2}{12 S_{xx}}\bigr)
\]
Let's make a computation when $N = 20$ and $M = 10$ in our model where $\sigma^2 = 9$.
...
Now let's look at estimating the MSE using bootstrapping. We are assuming that the `xs` are fixed here, and the random part is generating the `ys`. I will again compute the coefficients from the formulas rather than using `lm` because it is so much faster. 

```{r}
ys <- 1 + 2 * xs + rnorm(N, 0, sigma)
dd <- data.frame(xs, ys)
train_indices <- sample(1:N, replace = TRUE)
...
x_star <- test$xs
mean((coeffs_1 + coeffs_2 * x_star - test$ys)^2)
```

Our estimate of the MSE based on this one bootstrap resample is `r round(mean((coeffs_1 + coeffs_2 * x_star - test$ys)^2), 3)`. 

Now, we would do this about 100 times in order to estimate the MSE of any one model.

```{r,cache=TRUE}
boot_mse <- replicate(100, {
  ys <- 1 + 2 * xs + rnorm(N, 0, sigma)
  dd <- data.frame(xs, ys)
  train_indices <- sample(1:N, replace = TRUE)
  train <- dd[train_indices,]
...
To see how well our estimate is working, we need to repeat **this** many times!

```{r, cache=TRUE}
boot_data <- replicate(100, {
  boot_mse <- replicate(100, {
    ys <- 1 + 2 * xs + rnorm(N, 0, sigma)
    dd <- data.frame(xs, ys)
    train_indices <- sample(1:N, replace = TRUE)
...
In this case, the bootstrapping method tends to overestimate the true MSE.
...
Here, we see that 10-fold repeated CV tends to underestimate the true MSE, and the standard deviation of the estimator is quite high to boot. This indicates that, for this particular problem, bootstrapping seems to be the better way to go. 

```{exercise, label = "exercise-4-3"}
Use simulation to compute the expected value of the estimator for MSE when using LGOCV with 70% in group and 30% out of group in the above scenarios. How does it compare to the other two techniques?
```
