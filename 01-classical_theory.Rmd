...
1. If $X_1$ and $X_2$ are independent normal random variables with means $\mu_1, \mu_2$ and standard deviations $\sigma_1, \sigma_2$, then $X_1 + X_2$ is normal with mean $\mu_1 + \mu_2$ and standard deviation $\sqrt{\sigma_1^2 + \sigma_2^2}$.
...
To estimate the variance of an rv, we take a large sample and use the R function `var`, which computes the sample variance. This is not as straightorward from a theoretical standpoint as taking the `mean`, but we will ignore the difficulties. We know from Theorem \@
(thm:variancefacts) that the variance of $2X + 3Y$ shoule be $4 * 1 + 9 * 9 = 85$.  Let's check it out. We already have built our random sample, so we just use `var` on that.
...
```{r}
dd <- readxl::read_xls("../4870/data/world_happiness_data.xls") #This won't work for you!
...
\]
This is our estimate for $\beta_1$, so we add a $\hat\ $ to it to make this indication. In general, hats on tops of constants will indicate our estimates for the constants based on data.
\[
...
```{r}
plot(density(sim_data))
curve(dnorm(x, 2, sqrt(9/sum(xs^2))),add = T, col = 2 )
```
...
\]
in the happiness data set. We will also compare that to the values we got using SSE. We load the data again, clean it up, and remove the missing values and plot.

```{r}
dd <- readxl::read_xls("../4870/data/world_happiness_data.xls")

dd_2018 <- filter(dd, Year ==2018) 
dd_2018 <- janitor::clean_names(dd_2018)
dd_2018 <- select(dd_2018, life_ladder, log_gdp_per_capita, country_name)
dd_2018 <- dd_2018[complete.cases(dd_2018),]
plot(dd_2018$life_ladder, dd_2018$log_gdp_per_capita)
```

As a reminder, we can get the SSE estimator for $\beta_0$ and $\beta_1$ via `lm` as follows.

```{r}
lm(life_ladder ~ log_gdp_per_capita, data = dd_2018)
```
...
loss_two <- function(b0, b1) {
  sum(g(dd_2018$life_ladder - (b0 + b1 * dd_2018$log_gdp_per_capita)))
}
```
...
```{r}
ggplot(dd_2018, aes(x = log_gdp_per_capita, y = life_ladder)) +
  geom_point() +
...
We see that we don't get coefficients that are in between the two sets of coefficients that we computed previously. 
...
```{r}
library(MASS)
rlm(life_ladder ~ log_gdp_per_capita, data = dd_2018, psi = psi.bisquare)
```

We don't get a big change by doing it like this.
