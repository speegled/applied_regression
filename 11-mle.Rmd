...
tryCatch(optim(par = c(1, 1, 3, 1, 5), fn = like_fun, lower = c(-Inf, -Inf, 0.1, 0.1, 0.1), method = "L-BFGS-B"), 
         error = function(e) print("The covariance matrix is not positive definite"))
...
> If these were the true parameter values, how likely would our observed
> data be?

To answer that, we must compute the likelihood of the data under this
model.
...
``` {r}
pred_fixed_effect <- gamma + beta * ss$days
```
...
But in a random intercept model, observations from the same subject
...
Instead, we must use a **multivariate normal distribution** that
...
``` {r}
N <- length(ss$reaction)
(Z %*% t(Z))[1:20, 1:20] #this represents the dependence groupings
V <- tau2 * (Z %*% t(Z)) + sigma2 * diag(N)
V[1:20,1:20]
```
...
It creates an ( N `\times `{=tex}N ) matrix where:
...
### What does multiplying by `tau2` do?

It says:

> If two observations belong to the same subject, their covariance is (
> `\tau`{=tex}\^2 ).
...
### What does `sigma2 * diag(N)` do?

This adds residual variance to the diagonal.

That ensures that:

-   Each observation has total variance ( `\tau`{=tex}\^2 +
    `\sigma`{=tex}\^2 ).
-   Only observations from the same subject share covariance.

------------------------------------------------------------------------

### What does the final matrix represent?
...
``` {r}
dmvnorm(ss$reaction,
        mean  = pred_fixed_effect,
        sigma = V,
        log   = TRUE)
```

This computes the log-likelihood of the entire response vector at once.

Instead of multiplying many independent normal densities, we now
evaluate **one multivariate normal density** that accounts for:
...
-   Build the mean.
-   Carefully construct a covariance matrix that reflects the random
    effects.
-   Evaluate one multivariate normal density.

The hardest part is building the covariance matrix correctly.

Once that is done, the rest is just plugging into the multivariate
normal formula.
